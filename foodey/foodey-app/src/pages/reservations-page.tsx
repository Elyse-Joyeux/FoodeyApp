...
export function ReservationsPage() {
  ...
  const activeStatus = STATUS_TABS[active];
  const filtered = reservations.filter((r) => r.status === activeStatus);
  const countFor = (status: string) => reservations.filter((r) => r.status === status).length;
  ...
  return (
    <>
      ...
      <div className={styles.tabs}>
        {STATUS_TABS.map((t, i) => (
          <button key={t} className={`${styles.tab} ${i === active ? styles.tabActive : ''}`} onClick={() => setActive(i)}>
            {t} <span className={styles.tabCount}>{countFor(t)}</span>
          </button>
        ))}
      </div>
      ...
      <div className={styles.tableWrap}>
        ...
        {filtered.length === 0 ? (
          <div className={styles.empty}>No {activeStatus.toLowerCase()} reservations.</div>
        ) : (
          filtered.map((r) => (
