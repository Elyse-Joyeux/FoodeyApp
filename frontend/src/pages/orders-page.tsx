...
type OrderDraft = { customer: string; time: string; serviceState: Order['status']; items: OrderItem[] };

const emptyDraft = (): OrderDraft => ({
  customer: '', time: formatTime(new Date()), serviceState: 'In Process', items: [],
});
...
function OrderForm({ title, order, menu, onSave, onClose }: {
  ...
  const [draft, setDraft] = useState<OrderDraft>(order ? {
    ...
    time: order.time, serviceState: order.status, items: order.items.map((it) => ({ ...it })),
  } : emptyDraft());
